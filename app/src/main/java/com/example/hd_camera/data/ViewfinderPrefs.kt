...
/**
 * The switches on the Settings screen and the `showGrid` / `showZoomChips` props the design
 * document exposes are the same state — this is where it lives.
...
object ViewfinderPrefs {
    ...
    private fun defaultOf(key: String): Boolean = when (key) {
        KEY_GRID, KEY_ZOOM_CHIPS, KEY_VOLUME_SHUTTER -> true
        else -> false
    }
